...
**Author voice.** For posts authored by Mislav Sagovac, run the finished analytical draft through the global `mislav-humanizer` skill. It adapts the prose to Mislav's public-facing analytical voice without changing facts, numbers, citations, `[KUT]` markers, or evidence constraints. House style must not force a short format or a fixed number of figures. Do not run `qa-post` on Mislav-authored posts by default; use it only when Mislav explicitly requests it. Reproducibility checks, script reruns, chart inspection, and Quarto rendering remain required. Do not apply this author workflow to Luka Sikic's posts unless Luka explicitly asks.
