...
- The agents in `.claude/agents/` are called by the skills, not by you. `style-critic`, `editor`, `number-checker`, and `chart-critic` run inside `/qa-post`. `idea-generator` and `idea-critic` run inside `/brainstorm`. `chart-critic` also runs inside `/chart` when you ask for a review.
- For a quality pass run `/qa-post`. For ideas run `/brainstorm`. For framing run `/find-angle`. For a chart run `/chart`. After any correction run `/learn`.
